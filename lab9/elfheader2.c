...
if(argc==1){
	...
	printf("doesn't support (invalid)\n");
	  
      if(class==1)
	printf("Class: ELFCLASS32\n");
      if(class==2)
	printf("Class: ELFCLASS64\n");
	printf("doesn't support (64 bit)\n");
  ...
  while ((c = getopt (argc, argv, "Ss")) != -1){
    ...
    if( (fd = open(argv[2], O_RDWR)) < 0 ) {
      ...
      printf ("starting with 0x%x\n", header->e_shoff);
      Elf32_Shdr *secHeader = (Elf32_Shdr *)(map_start + header->e_shoff);
      char *secnames;
      secnames = map_start + secHeader[header->e_shstrndx].sh_offset;
      int i=0;
      for(i=0; i<header->e_shnum; i++){
	printf ("Section header name: %s              type: %s           Address: %s\n",secnames + secHeader[i].sh_name, secnames + secHeader[i].sh_type, secnames + secHeader[i].sh_addr);
      }
    }
